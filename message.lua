...
local IndexUIColor = {
    ...
    ["Logo Image"] = "rbxassetid://13377314456",
...
function SaveCustomUISettings()
    local HttpService = game:GetService("HttpService")
    if not isfolder("MMB HUB") then
        makefolder("MMB HUB")
    end
    writefile("MMB HUB/" .. SaveCustomFileName, HttpService:JSONEncode(CorrectTable(SettingsRac)))
end
...
function ReadCustomUISetting() 
    local s,e = pcall(function() 
        ...
        return HttpService:JSONDecode(readfile("MMB HUB/" .. SaveCustomFileName))
    end)
    if s then return e 
    else
...
Library_Function.Gui.Name = 'MMB HUB GUI'
...
Library_Function.NotiGui.Name = 'MMB HUB Notification'
...
function Library.CreateNoti(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. color .. ")\">MMB HUB</font> "..getgenv().TitleNameNoti
...
function Library.CreateMain(Setting)
    ...
    local TitleNameMain = tostring(Setting.Title) or "MMB HUB"
    getgenv().MainDesc = "V3"
    ...
    function Library_Function.ReloadMain(v)
        ...
        local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
        local color = colorR .. ',' .. colorG .. ',' .. colorB
        TextLabelMain.Text = "<font color=\"rgb("..color..")\">MMB HUB</font> " .. getgenv().MainDesc
        table.insert(UpdateCallBack["Title Text Color"],function() 
            maingui.ImageColor3 = getgenv().UIColor['Title Text Color']
            local colorR = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[1])
            local colorG = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[2])
